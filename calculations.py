...
#returns intersection point between two given lines([P11, P12] and [P21, P22])
...
#returns vector of given line segment (P1 - start point, P2 - end point)
def GetVector(P1, P2) :
    vx = P2[0] - P1[0]
    vy = P2[1] - P1[1]
    return [vx, vy]
...
def Fact(n):
    ...
    for i in range(n):
        f = f * (i + 1)
    return f
#returns binominal coefficients
def BinCoef(n, k):
    return int(Fact(n)/(Fact(k) * Fact(n - k)))
#transform points(move and rotate)
